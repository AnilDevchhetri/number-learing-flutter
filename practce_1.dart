...
''';
  ...
  void load_next_question() {
    ...
    raw_option_list.shuffle(random);
    ...
    setState(() {
      ...
      option_list = raw_option_list;
      option_used_list = List<bool>.filled(raw_option_list.length, false);
  ...
  void place_option_in_answer(int option_index) {
    ...
    if (option_used_list[option_index]) return;

    final int empty_index = selected_answer_list.indexOf(null);
    if (empty_index == -1) return;

    setState(() {
      selected_answer_list[empty_index] = option_list[option_index];
      option_used_list[option_index] = true;
    });

    check_if_answer_complete();
  }

  void place_dragged_text_in_answer(String dragged_text) {
    if (is_checking) return;

    final int option_index = option_list.indexWhere((item) {
      return item == dragged_text &&
          option_used_list[option_list.indexOf(item)] == false;
    });
    ...
    setState(() {
      selected_answer_list[empty_index] = dragged_text;
      option_used_list[option_index] = true;
    });
  ...
  void remove_answer_at(int answer_index) {
    ...
    final int option_index = option_list.indexWhere((item) {
      if (item != removed_text) return false;
      return option_used_list[option_list.indexOf(item)] == true;
    });
    ...
    setState(() {
      ...
      option_used_list[option_index] = false;
    });
  }
  ...
  Future<void> check_answer() async {
    ...
    setState(() {
      ...
      if (is_correct) {
        correct_count++;
        feedback_text = 'Correct! すごい！';
        feedback_color = const Color(0xFF1FA35B);
      } else {
        wrong_count++;
        feedback_text = 'Wrong → ${current_item!['reading']}';
  ...
  void reset_current_answer() {
    if (is_checking) return;

    setState(() {
      for (int i = 0; i < selected_answer_list.length; i++) {
        selected_answer_list[i] = null;
      }
      for (int i = 0; i < option_used_list.length; i++) {
        option_used_list[i] = false;
      }
    });
  }
  ...
  Widget build_answer_slot(int index) {
    ...
    return DragTarget<String>(
      onWillAcceptWithDetails: (details) => !is_filled && !is_checking,
      onAcceptWithDetails: (details) {
        place_dragged_text_in_answer(details.data);
      },
  ...
  Widget build_option_tile(int index, bool is_small_device) {
    final bool is_used = option_used_list[index];
    final String option_text = option_list[index];
      ...
      children: [
        Expanded(
          child: OutlinedButton(
            onPressed: reset_current_answer,
